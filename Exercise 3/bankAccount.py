# FIle name: bankAccount.py
# Author: Jesse Malinen
# Description: script for BankAccount class
    # balance is private
    # two files
    # two data attributes
    # __str__

class BankAccount:
    
    # The __init__ method accepts an argument
    # for the account's balance. It is assigned to
    # the __balance attribute.
    def __init__(self, bal, owner):
        self.__balance = bal
        self.__owner = owner
    
    
    # The deposit method makes a deposit to the account.
    def deposit(self, amount):
        self.__balance += amount
        
        
    # The withdraw method withdraws an amount from the account.
    def withdraw(self, amount):
        if self.__balance >= amount:
            self.__balance -= amount
        else:
            print("Error: Insufficient funds.")
    
    
    # The get_balance method returns the account balance.
    def get_balance(self):
        return self.__balance

    # The get_owner method returns the bank account owner
    def get_owner(self):
        return self.__owner
    
    # the __str__ method returns a string
    # indicating the object's state.
    def __str__(self):
        return f""" Balance: {self.__balance} Owner: {self.__owner}""" 
    